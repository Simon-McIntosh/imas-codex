...
- **Pipeline seats:** each seat's production model is its own `[sn-*]` section — `[sn-compose]`, `[sn-docs]`, `[sn-refine]`, `[sn-parent-enrich]` (`get_model("sn-<seat>")`), and the reviewer chains in `[sn-review.names]` / `[sn-review.docs]` (+ named profiles under `[sn-review.names.profiles.*]`), read via `get_sn_review_names_models()` / `get_sn_review_docs_models()`.
