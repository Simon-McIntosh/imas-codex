...
def test_grammar_segment_reconciliation_is_dry_run_by_default(
    tmp_path: Path,
) -> None:
    manifest = tmp_path / "grammar-segments.json"
    manifest.write_text("{}", encoding="utf-8")
    receipt = {"mode": "dry_run", "counts": {"planned": 5, "applied": 0}}
    with patch(
        "imas_codex.standard_names.grammar_segment_reconciliation.reconcile_grammar_segments",
        return_value=receipt,
    ) as reconcile:
        result = CliRunner().invoke(
            sn,
            [
                "reconcile-grammar-segments",
                "--manifest",
                str(manifest),
                "--reason",
                "align exact grammar projections",
            ],
        )
...
def test_grammar_segment_reconciliation_apply_is_hash_bound(tmp_path: Path) -> None:
    manifest = tmp_path / "grammar-segments.json"
    manifest.write_text("{}", encoding="utf-8")
    with patch(
        "imas_codex.standard_names.grammar_segment_reconciliation.reconcile_grammar_segments",
        return_value={"mode": "applied"},
    ) as reconcile:
        result = CliRunner().invoke(
            sn,
            [
...
def test_grammar_segment_reconciliation_apply_requires_hash(tmp_path: Path) -> None:
    manifest = tmp_path / "grammar-segments.json"
    manifest.write_text("{}", encoding="utf-8")
    with patch(
        "imas_codex.standard_names.grammar_segment_reconciliation.reconcile_grammar_segments"
