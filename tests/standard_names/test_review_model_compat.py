...
# ---------------------------------------------------------------------------
# 4. Anthropic structured output routes via pydantic-class (native tool-use),
#    not the json_schema response_format dict that OpenRouter rejects as
#    "Schema is too complex" once a $defs object exceeds ~13 properties.
# ---------------------------------------------------------------------------


def _build_kwargs_with_format(model: str, response_format):
    from imas_codex.discovery.base.llm import _build_kwargs
...
@pytest.mark.parametrize(
    "model,expect_pydantic_class",
    [
        ("openrouter/anthropic/claude-opus-4.8", True),
        ("anthropic/claude-sonnet-5", True),
        ("openrouter/openai/gpt-5.5", False),
        ("openrouter/google/gemini-3.1-pro-preview", False),
    ],
)
def test_anthropic_structured_output_uses_pydantic_class_route(
    model, expect_pydantic_class
):
    ...
    kwargs = _build_kwargs_with_format(model, _Resp)
    rf = kwargs.get("response_format")
    if expect_pydantic_class:
        # Anthropic: hand litellm the Pydantic class → native tool-use, which
        # has no per-$defs property limit (RefinedName.segments = 14 props).
        assert rf is _Resp, (
            f"Anthropic model {model} must pass the Pydantic class as "
            f"response_format (native tool-use), got {type(rf)}: {rf!r}"
        )
    else:
        # Others: explicit non-strict json_schema dict (reliable via proxy).
        assert isinstance(rf, dict) and rf.get("type") == "json_schema", (
            f"non-Anthropic model {model} must use a json_schema dict, got {rf!r}"
        )
