...
class TestClaimOnlyDrafted:
    ...
    def test_claim_requires_current_valid_status(self):
        """Pending or quarantined candidates never enter paid name review."""
        captured: list[str] = []
        ...
        assert len(captured) == 1
        assert "sn.validation_status = 'valid'" in captured[0]
...
class TestPersistToExhausted:
    def test_persist_to_reviewed_at_escalator_attempt(self):
        ...
        process_refine_name_batch, which fires at
        chain_length == rotation_cap-1.  The SN must stay 'reviewed' so that
        final escalated refine can run.
        """
        gc = _mock_gc_query(
            return_values=[
                [{"chain_length": 2}],
