...
- The required `sn retry --failed` preflight and live operation both refused the entire sample because those sources are already claimable rather than failed or attempt-capped: `eligible: 0 of 12 requested source(s)`, then `retried: 0 of 12 requested source(s)` and `refused: 12 source path(s) were missing or not failed/attempt-capped`.
- The exact focused run seeded only the 12 sample paths, then failed closed before the first source claim or model call: `The configured local Standard Names compose endpoint is required for generation but is unavailable (HTTP 503). Restore the Ambix vLLM service and retry; no paid provider fallback will be used.` The router's anonymous catalog probe independently returned HTTP 503 with `no upstream catalogs are reachable`.
- Consequently the recovery metric is **0 grammar-valid names produced / 0 sources tried**, not 0/12. All 12 were scoped, but none reached composition. Actual spend is **$0.00 / $5.00 capped**. A source-level recovery rate cannot honestly be estimated until the local catalog returns at least one model.
- Scope proof: run scope `12ff2fff-0ada-493a-b973-cffad99158fa` is present on exactly the 12 sampled sources and on **0** sources outside the sample. Every sampled source remains unclaimed, with its source status and attempt counter unchanged; no new `PRODUCED_NAME` edge exists.

Full terminal transcript: `/home/ITER/mcintos/.config/reckon/crew/runs/r-20260915T112100248211-n-swcr-the-grammar-invalid-cohort-is-resized-after-the-classifier-repair/focused-sn-run.log`.

## Sample receipt

| DD source path | Historical grammar-invalid name and before stage | Source state before to after | After name stage |
|---|---|---|---|
| `camera_ir/fibre_bundle/geometry/surface` | `surface_area_of_optical_element` (exhausted) | `extracted`, attempts `0` to unchanged; run scope stamped | none - no source claim and no current name |
| `cryostat/description_2d/cryostat/unit/annular/outline_outer/z` | `vertical_outline_of_cryostat` (exhausted) | `extracted`, attempts `0` to unchanged; run scope stamped | none - no source claim and no current name |
| `gyrokinetics_local/linear/wavevector/eigenmode/fields/b_field_parallel_perturbed_norm` | `parallel_normalized_perturbed_magnetic_field_amplitude` (exhausted) | `extracted`, attempts `2` to unchanged; run scope stamped | none - no source claim and no current name |
| `gyrokinetics_local/non_linear/fields_intensity_1d/b_field_parallel_perturbed_norm` | `parallel_normalized_perturbed_magnetic_field` (exhausted) | `extracted`, attempts `0` to unchanged; run scope stamped | none - no source claim and no current name |
| `neutron_diagnostic/detector/energy_band/lower_bound` | `lower_bound_energy_of_neutron_detector` (exhausted) | `extracted`, attempts `0` to unchanged; run scope stamped | none - no source claim and no current name |
| `plasma_transport/model/ggd/neutral/energy/v_parallel/values` | `parallel_neutral_species_energy_convection_velocity` (exhausted) | `extracted`, attempts `0` to unchanged; run scope stamped | none - no source claim and no current name |
| `runaway_electrons/profiles_1d/e_field_critical` | `runaway_electron_critical_electric_field` (exhausted) | `extracted`, attempts `0` to unchanged; run scope stamped | none - no source claim and no current name |
| `spectrometer_visible/channel/filter_spectrometer/photoelectric_voltage` | `voltage_of_spectrometer` (exhausted) | `extracted`, attempts `0` to unchanged; run scope stamped | none - no source claim and no current name |
| `summary/gas_injection_prefill/tritium/value` | `tritium_prefill_count` (exhausted) | `extracted`, attempts `0` to unchanged; run scope stamped | none - no source claim and no current name |
| `summary/pedestal_fits/linear/pressure_electron/d_dpsi_norm_max/value` | `gradient_of_electron_pressure` (exhausted) | `extracted`, attempts `0` to unchanged; run scope stamped | none - no source claim and no current name |
| `wall/description_ggd/ggd/energy_fluxes/current/emitted/values` | `energy_flux_due_to_eddy_current` (exhausted) | `extracted`, attempts `0` to unchanged; run scope stamped | none - no source claim and no current name |
| `waves/coherent_wave/profiles_1d/current_phi_inside_n_phi` | `per_toroidal_mode_current_due_to_wave_driven_current_drive` (superseded) | `extracted`, attempts `2` to unchanged; run scope stamped | none - no source claim and no current name |
...
The measurement completes only when `GET http://98dci4-gpu-0003:18802/v1/models` returns HTTP 200 with at least one model. Resume the same 12 paths; do not add sources and do not use paid fallback. The retained run-scope stamp is an operational residue from the pre-pool endpoint refusal and should be overwritten or cleared by the sanctioned focused-run lifecycle, not by ad-hoc Cypher.
