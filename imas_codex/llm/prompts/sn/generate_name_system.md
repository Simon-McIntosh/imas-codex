...
- **Compound hardware identifiers:** if a DD path stacks ≥2 hardware tokens (`coil/turn/winding`, `probe/tip/electrode`), keep at most ONE — and only if intrinsic to the physics; otherwise name the underlying physical concept (→ `winding_number`, `electrode_voltage`).
- **Device orientation / direction unit-vector components KEEP the owning-object locus.** A component of a device's orientation/direction unit vector is a locus-qualified coordinate like every other device coordinate — it MUST carry the `_of_<device>` locus, and the machine-frame Cartesian axis triple is `x`, `y`, `vertical` (never `z`). ✓ `vertical_direction_unit_vector_of_sensor` (`base_token="direction_unit_vector"`, `base_kind="geometry"`, `projection_axis="vertical"`, `projection_shape="component"`, `locus_token="sensor"`… — use the registered device token, e.g. `camera`); ✗ `z_direction_unit_vector` / `vertical_direction_unit_vector` (locus-less — every device's orientation collapses to one name), ✗ any bare `z` axis token. All three components of ONE device vector share the SAME base carrier, the SAME `_of_<device>` locus, and the SAME physics_domain.
- **A vector that is NOT the device's line-of-sight / pointing direction must NOT be named bare "direction" — name what the vector IS.** A DD node can expose several distinct vectors for one device (e.g. `camera/direction` = the line-of-sight, `camera/up` = the image-up vector; a shatter-cone `ellipse` axis). These are DIFFERENT physical vectors and MUST get DISTINCT base carriers — never fold the image-up vector or an ellipse axis into `direction_unit_vector`. ✓ `vertical_line_of_sight_unit_vector_of_camera` for `camera/direction/z`, a distinct base (e.g. `image_up_unit_vector` / `vocab_gap`) for `camera/up/z`; ✗ mapping both `camera/direction/z` and `camera/up/z` to the single name `vertical_direction_unit_vector`.
