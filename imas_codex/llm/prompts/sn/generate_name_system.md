...
- **The test:** "does the entity *have* this quantity as a defining attribute?" If yes → `of`. If the quantity is a field merely sampled there → `at`. ✗ `poloidal_magnetic_flux_of_plasma_boundary` (flux is a field, evaluated AT the boundary); ✗ `electron_density_of_pedestal_top` (density is a field evaluated AT the pedestal top, not an intrinsic property of it).
...
- **Canonical locus tokens — never a synonym** (HARD RULE). Several features have multiple literature names; the catalog uses exactly ONE per concept even when the DD path/description uses an alias. Choose the canonical `locus_token`, THEN apply the of/at test:
  ...
  | `pedestal_top` (value) · `pedestal_maximum` (max-gradient / flux-position) | bare `pedestal`, `pedestal_region`, `edge_pedestal` |
...
- **Pedestal locus — two registered positions, never bare `pedestal` (HARD RULE).** The pedestal is a LOCUS (`locus_relation="at"`), not a zone prefix, and it resolves to ONE of two registered position tokens by what the quantity measures:
  - **Value-at-the-pedestal quantities** (the pedestal *value* of a profile field — temperature, density, pressure) → `pedestal_top`. ✓ `electron_temperature_at_pedestal_top`, `electron_density_at_pedestal_top`, `electron_pressure_at_pedestal_top`.
  - **Maximum-gradient quantities and flux/position quantities** (the steepest-gradient locus, and coordinates/fluxes that *locate* the pedestal) → the mid-pedestal locus `pedestal_maximum`. ✓ `maximum_electron_pressure_gradient_at_pedestal_maximum`, `normalized_poloidal_flux_coordinate_of_pedestal` (the flux coordinate that fixes the pedestal position).
  - Never emit a bare `pedestal` locus (`✗ electron_density_at_pedestal`) or a `pedestal` zone prefix (`✗ pedestal_electron_temperature`) — both collapse the two distinct loci onto one ambiguous name.
- **Limiter loci — three distinct positions, do not conflate (HARD RULE).** A "limiter" locus resolves to one of three registered positions by the physics:
  - **`limiter`** — the SOL-adjacent limiter POINT (a value read at the limiter). ✓ `electron_temperature_at_limiter`, `electron_density_at_limiter`.
  - **`limiter` with an `over`/distribution locus (`along_limiter`)** — a distribution or peak taken OVER the limiter surface (e.g. a peak power density along the limiter). ✓ `peak_power_density_along_limiter`.
  - **`active_limiter_point`** — the separatrix–limiter tangency (the contact point where the plasma boundary touches the active limiter). ✓ `radial_coordinate_of_active_limiter_point`.
  Pick the one that matches the source; do NOT use a bare `limiter` for a surface distribution or for the tangency contact.
