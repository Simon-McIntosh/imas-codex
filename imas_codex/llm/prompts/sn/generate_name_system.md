...
These three closed segments occupy fixed slots in the canonical prefix order — between `subject`/`device` and the `base` — but, like every prefix token in this IR, **you emit them as `qualifiers[]` entries**; ISN's composer puts them in canonical order. Their token lists are in the segment reference above; use only registered tokens. Canonical order among the prefixes is `… → subject → device → zone → qualifier(s) → channel_qualifier → channel → base …` — a refined qualifier (`implicit`, `incident`, `effective`, `deposited`, …) scopes over the WHOLE channel phrase and therefore precedes the channel, English adjective order: ✓ `implicit_energy_source_rate` (`qualifiers=["implicit", "energy"]`, `base_token="source_rate"`), ✓ `incident_kinetic_energy_flux_at_wall` (`qualifiers=["incident", "kinetic", "energy"]`), ✓ `ion_state_implicit_energy_source_rate`; ✗ `energy_implicit_source_rate`, ✗ `kinetic_energy_incident_flux`.
