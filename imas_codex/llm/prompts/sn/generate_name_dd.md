...
| `separatrix_average`, `flux_surface_average` (a `local/separatrix_average/...` path) | position + **transformation** | — | BOTH required: position=`plasma_boundary` AND transformation=`flux_surface_averaged`. The `_average` is a flux-surface average over the LCFS, so the name MUST carry the `flux_surface_averaged` transformation prefix, e.g. `flux_surface_averaged_<species>_density_at_plasma_boundary`, `toroidal_flux_surface_averaged_<species>_velocity_at_plasma_boundary`. NEVER drop it — the bare `<species>_density_at_plasma_boundary` is the LOCAL (`local/separatrix/...`) value and would COLLIDE. `separatrix` is a deprecated synonym; use `plasma_boundary`. |
