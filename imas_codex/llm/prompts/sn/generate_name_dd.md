...
| `separatrix_average`, `flux_surface_average` | position | — | Split: position=`plasma_boundary` (canonical LCFS locus — NOT `separatrix`, which is a deprecated synonym) + transformation (or physical_base compound) |
| `separatrix` (bare, in a `local/separatrix/...` path) | position | — | Use position=`plasma_boundary` (the bare separatrix IS the plasma boundary / LCFS; reserve `secondary_separatrix` etc. for genuinely distinct qualified surfaces) |
