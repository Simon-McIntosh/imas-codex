...
def get_llm_module_dir() -> Path:
    """Get the LLM module directory for generated context models."""
    return get_project_root() / "imas_codex" / "llm"


def _harden_prompt_context_models(generated_code: str) -> str:
    """Make generated prompt models immutable and coercion-free."""
    assignment_marker = "validate_assignment = True,"
    strict_marker = "strict = False,"
    if assignment_marker not in generated_code or strict_marker not in generated_code:
        raise ValueError("Generated Pydantic base-model configuration was not found")
    return generated_code.replace(
        assignment_marker,
        f"{assignment_marker}\n        frozen = True,",
        1,
    ).replace(strict_marker, "strict = True,", 1)
...
def build_models(
    ...
    3. Facility configuration models from schemas/facility_config.yaml
    4. Typed prompt context models from schemas/llm_context.yaml
...
'''
        ...
        # Generate provider-independent prompt context models. This schema is
        # deliberately separate from graph schemas because envelopes are
        # request data and are never graph nodes.
        context_schema_file = schemas_dir / "llm_context.yaml"
        context_output_file = get_llm_module_dir() / "context_models.py"

        if context_schema_file.exists():
            needs_regen = not context_output_file.exists() or force
            if context_output_file.exists() and not force:
                needs_regen = (
                    context_schema_file.stat().st_mtime
                    > context_output_file.stat().st_mtime
                )
                if needs_regen:
                    logger.info(
                        "Prompt context schema newer than models, regenerating..."
                    )

            if needs_regen:
                logger.info(
                    f"Generating prompt context models from {context_schema_file}"
                )
                if not dry_run:
                    cmd = ["gen-pydantic", str(context_schema_file)]
                    logger.debug(f"Running command: {' '.join(cmd)}")
                    try:
                        result = subprocess.run(
                            cmd,
                        ...
                        logger.error(
                            "gen-pydantic timed out after 120s for prompt context"
                        )
                        click.echo("Error: gen-pydantic timed out after 120s", err=True)
                        return 1
                    ...
                    if result.returncode != 0:
                        logger.error(
                            f"gen-pydantic failed for prompt context: {result.stderr}"
                        )
                        click.echo(
                            f"Error: gen-pydantic failed:\n{result.stderr}", err=True
                        )
                    ...
                    header = '''"""
Typed LLM Prompt Context Pydantic Models.

AUTO-GENERATED from imas_codex/schemas/llm_context.yaml
DO NOT EDIT THIS FILE DIRECTLY - edit the LinkML schema instead.
...
'''
                    generated_code = _harden_prompt_context_models(
                        header + result.stdout
                    )
                    context_output_file.write_text(generated_code, encoding="utf-8")
                    logger.info(
                        f"Generated prompt context models at {context_output_file}"
                    )
                    click.echo(f"Generated: {context_output_file}")
                else:
                    click.echo(f"Would generate: {context_output_file}")
            else:
                logger.info(
                    f"Prompt context models up to date at {context_output_file}"
                )
                click.echo(f"Prompt context models up to date: {context_output_file}")
        else:
            logger.debug(f"Prompt context schema not found: {context_schema_file}")
